...
namespace SheldonClones
{
    ...
    /* Дополнительный патч для обеспечения того, что клоны всегда имеют футболки при загрузке сохранения
    [HarmonyPatch(typeof(Game), "LoadGame")]
    public static class Game_LoadGame_Patch
    {
        public static void Postfix()
        {
            // Проходим по всем картам и проверяем клонов Шелдона
            foreach (Map map in Find.Maps)
            {
                var sheldonClones = map.mapPawns.AllPawns
                    .Where(p => p.def?.defName == "SheldonClone" && p.RaceProps.Humanlike);

                foreach (Pawn clone in sheldonClones)
                {
                    SheldonClothingHelper.EnsureSheldonHasShirt(clone);
                }
            }
        }
    }
