    ...
    public class GameComponent_SheldonWatcher : WorldComponent
    {
        ...
        public void ApplyStrikeToInitiator(Pawn sheldonClone, Pawn initiator)
        {
            ...
            if (existingStrike != null)
            {
                ...
                {
                    // Сбрасываем таймер decay:
                    var decayComp = existingStrike.TryGetComp<HediffComp_SheldonStrikeDecay>();
                    if (decayComp != null)
                    {
                        decayComp.ResetDecayTimer();
                    }
                    // Можно добавить лог:
                    Log.Message($"{initiator.LabelShort} уже имеет 3 уровень страйка от {sheldonClone.LabelShort}. Таймер страйка сброшен.");
                    return;
                }
        ...
        public override void ExposeData()
        {
            ...
            // 3) После загрузки — из списка строим реальный словарь
