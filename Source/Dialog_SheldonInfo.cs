...
namespace SheldonClones
{
    public class Dialog_SheldonInfo : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            // Секция: Игнорируемые пешки
            var ignoredPawns = watcher?.GetIgnoredPawns(owner) ?? new List<Pawn>();

            if (ignoredPawns.Count > 0)
            {
                curY += 10f;
                Widgets.Label(new Rect(inRect.x, inRect.y + curY, inRect.width, 24f), "Игнорируемые пешки (3 страйка):");
                curY += 22f;

                foreach (var ignored in ignoredPawns)
                {
                    string line = $"{ignored.LabelShort} (полностью игнорируется)";
                    Widgets.Label(new Rect(inRect.x + 20f, inRect.y + curY, inRect.width - 20f, 20f), line);
                    curY += 18f;
                }
            }
