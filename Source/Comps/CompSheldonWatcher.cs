...
{
    public class CompSheldonWatcher : WorldComponent
    {
        ...
        public CompSheldonWatcher(World world) : base(world) { }
...

        {
            ...
            // Сбрасываем все накопленные попытки выселения с места клона при старте/загрузке
            Patch_CarryIngestibleToChewSpot.ClearAllEvictionAttempts();
            SocialRelaxSeatAssigner.ClearAllSocialEvictionAttempts();
        }
        ...
        public List<Pawn> GetIgnoredPawns(Pawn sheldon)
        {
            if (sheldon == null)
                return new List<Pawn>();
            if (ignoredPawns != null && ignoredPawns.TryGetValue(sheldon, out var list))
                return list;
            return new List<Pawn>();
        }

        public void RemoveIgnoredPawn(Pawn sheldonClone, Pawn target)
        {
            if (ignoredPawns.TryGetValue(sheldonClone, out var list))
            {
                list.Remove(target);
                if (list.Count == 0)
                {
                    ignoredPawns.Remove(sheldonClone);
                }
            }
